 ...
 * @return count
 */
function evoting_get_feedback_list()
{
    global $DB;

    $fb = $DB->get_records_sql("
    SELECT feedback FROM {evoting_answers}
    WHERE feedback is not null");
    $feed = array_values($fb);
 ...
 * @return mixed
 */
function evoting_vote($userVoteId, $optionId, $idPoll, $lang) //KK
{
    global $DB;
    //$votingfeedback
    ...
    if ($poll[0]->publish == true) {
		...
		AND a.uservoteid = ?", array('evotingquestionid' => $idQuestion, 'uservoteid' => $userVoteId));
                    ...
                    //$newanswer->feedback = $votingfeedback //KK
